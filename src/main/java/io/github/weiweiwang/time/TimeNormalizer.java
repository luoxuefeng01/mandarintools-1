 ...
 * 复旦NLP源码，Deprecated，请使用TimeEntityRecognizer
 * @author 邬桐, 曹零
...
public class TimeNormalizer implements Serializable {
    ...
    public TimeNormalizer() {
        if (patterns == null) {
            try {
                InputStream in = getClass().getResourceAsStream("/TimeExp.zip");
    ...
    public TimeUnit[] parse(String target) {
        return parse(target, new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss")
